...
describe("Scenario B: Inferred dispatch", () => {
  it("PASS (quality: manual review)", async () => {
    ...
    try {
      ...
      const inferenceRun = await waitForHubMessage(
        harness.hub,
        (message) =>
          message.intent === "run" &&
          message.payload.content.includes("Convert the TaskSpec below into a dispatch DAG.")
      );
      expect(inferenceRun.payload.content).toContain("Inspect the repository.");

      await harness.hub.sendResult(inferenceRun, {
        source: "claude",
        content: [
          "```json",
          "[",
          '  {"task_id":"inspect","instruction":"Inspect the repository","depends_on":[]},',
          '  {"task_id":"summarize","instruction":"Draft a release summary","depends_on":["inspect"]}',
          "]",
          "```"
        ].join("\n")
      });

      const inspectRun = await waitForHubMessage(
        harness.hub,
        (message) => message.intent === "run" && message.payload.content === "Inspect the repository"
      );
      await harness.hub.sendResult(inspectRun, {
        source: "claude",
        content: "Inspection complete"
      });

      const summaryRun = await waitForHubMessage(
        harness.hub,
        (message) => message.intent === "run" && message.payload.content === "Draft a release summary"
      );
      await harness.hub.sendResult(summaryRun, {
        source: "codex",
      ...
      await vi.waitFor(async () => {
        ...
        const role = state.roles.find((entry) => entry.threadId === "dispatcher-b");
      ...
      const detail = await harness.requestJson<{
        status: string;
        taskspec?: string;
        tasks: Array<{ task_id: string; instruction: string; status: string }>;
      }>("GET", "/api/role/dispatcher-b");
      expect(detail.status).toBe("completed");
      expect(detail.taskspec).toContain("Inspect the repository.");
      expect(detail.tasks.map((task) => ({ task_id: task.task_id, status: task.status }))).toEqual([
        { task_id: "inspect", status: "done" },
        { task_id: "summarize", status: "done" }
      ]);
      expect(detail.tasks.map((task) => task.instruction)).toEqual([
