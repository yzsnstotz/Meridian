...
describe("Scenario D: Restart recovery", () => {
  it("PASS", async () => {
    const baseDir = await fs.mkdtemp("/tmp/meridian-roles-scenario-d-");
    const stateFilePath = path.join(baseDir, "state.json");

    const firstHarness = await startHarness({ baseDir, cleanupDirOnClose: false });
    let runningTraceId = "";

    try {
      await firstHarness.requestJson("POST", "/api/role", {
        thread_id: "dispatcher-d",
        user_reply_channel: {
          channel: "web",
          chat_id: "web:recovery"
        },
        tasks: [
          {
            task_id: "A",
            instruction: "Run the first leg",
            depends_on: [],
            target_thread_id: "claude_01"
          },
          {
            task_id: "B",
            instruction: "Run the second leg",
            depends_on: ["A"],
      ...
      const firstRun = await waitForHubMessage(
        firstHarness.hub,
        (message) => message.intent === "run" && message.payload.content === "Run the first leg"
      );
      await firstHarness.hub.sendResult(firstRun, {
        source: "claude",
        content: "A complete"
      });
      ...
      const secondRun = await waitForHubMessage(
        firstHarness.hub,
        (message) => message.intent === "run" && message.payload.content === "Run the second leg"
      );
      runningTraceId = secondRun.trace_id;

      const inFlightState = await firstHarness.readState();
      const role = inFlightState.roles.find((entry) => entry.threadId === "dispatcher-d");
      const config = role?.config as { tasks?: Array<{ task_id: string; status: string; result_trace_id?: string }> };
      expect(role?.status).toBe("active");
      expect(config.tasks?.map((task) => ({ task_id: task.task_id, status: task.status }))).toEqual([
        { task_id: "A", status: "done" },
        { task_id: "B", status: "running" }
      ]);
      expect(config.tasks?.[1]?.result_trace_id).toBe(runningTraceId);
    } finally {
      await firstHarness.close();
    }

    const persistedState = await readStateFile(stateFilePath);
    const secondHarness = await startHarness({ baseDir, cleanupDirOnClose: false });

    try {
      await rehydratePersistedRoles(secondHarness, persistedState);

      await vi.waitFor(() => {
        const runMessages = secondHarness.hub.messages.filter(
          (message) => message.intent === "run" && message.suppress_reply !== true
        );
      ...
      await sendSocketPayload("/tmp/meridian-roles.sock", {
        ...
        source: "codex",
        status: "success",
        content: "B resumed complete",
        attachments: [],
        timestamp: new Date().toISOString()
      });

      await vi.waitFor(async () => {
        const state = await secondHarness.readState();
        const role = state.roles.find((entry) => entry.threadId === "dispatcher-d");
...
async function rehydratePersistedRoles(harness: ScenarioHarness, state: AppState): Promise<void> {
