...
const dispatchScheduleStopTool: ToolDefinition = {
  name: "dispatch-schedule-stop",
  description: "Disable future scheduler cycles (does not kill active run)",
  params: {
    scheduler: {
      type: "string",
      required: true,
  ...
  async execute(params: Record<string, string>): Promise<ToolResult> {
    ...
    try {
      const response = await postRolesServiceJson<unknown>(
        `/api/scheduler/${encodeURIComponent(schedulerId)}/stop`,
        {}
      );
...
export default dispatchScheduleStopTool;
