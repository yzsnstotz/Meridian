...
test("INT-01: Unix Socket IPC — spawn creates socket file, run succeeds, kill removes socket", async () => {
  ...
  try {
    const threadId = "cursor_01";
    const agentSocketPath = path.join(tempDir, `agentapi-${threadId}.sock`);

    const spawnRes = await sendHubIpc(hubSocketPath, buildHubMessage({ intent: "spawn", target: "cursor" }));
    assert.equal(spawnRes.status, "success", JSON.stringify(spawnRes));
