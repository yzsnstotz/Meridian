...
import { deriveBuiltinCallerKey } from "../../../src/shared/caller-bootstrap";
import { wrapHubMessage } from "../../../src/shared/caller-wire";
...
export async function sendHubIpc(socketPath: string, message: HubMessage): Promise<HubResult> {
  ...
  const frame = wrapHubMessage(message, {
    caller_id: callerId,
    caller_key: deriveBuiltinCallerKey(callerId),
    caller_label: "Meridian CLI"
  });
  return sendIpcRequest<typeof frame, HubResult>(socketPath, frame);
}
