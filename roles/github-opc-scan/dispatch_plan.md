...
Runtime workers are thin CLI orchestrators. They use the scheduler-provided `SCAN_RUN_ID`; manual dry runs may fall back to `daily-YYYY-MM-DD` in the scheduler timezone. They must run only the CLI subcommand specified by their worker file, validate JSON summary output, report the result, and stop.
