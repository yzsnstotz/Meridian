...
| `/api/system` | `GET` | Returns the supervisor materialized process projection for the product UI. | Reads `<stateDir>/supervisor.json` without writing or acquiring execution authority; missing/unreadable state returns `503`. | `packages/runtime/src/web/server.ts` |
