...
- **Implementation**: The registry clones records on read and write, logs register/unregister transitions, and exposes targeted patch helpers for status, auto-approve, stream capability, Codex session ID, and caller tracking. `setSpawnedBy(threadId, caller)` records who spawned an instance; `setCaller(threadId, caller, ts)` atomically updates `last_caller` and `last_caller_at` on mutate-intent dispatch. Both are no-ops for unknown threads. [UPDATED 2026-05-05]
