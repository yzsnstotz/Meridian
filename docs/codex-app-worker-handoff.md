...
9. Once native App state and the exact turn event confirm terminal, write `{threadId,turnId,status,text}` to a private result file and `complete ID codex-app-controller FILE`. `status` must be completed, failed or interrupted; `text` is the real final result or authentic native terminal error. A `task_complete` with an error object is failed even when `last_agent_message` is null; the actual nonempty error message takes precedence over final text. A malformed or empty error message, or an empty final without error, requires reconciliation, never invented success. Meridian then receives the real result and applies its normal report/validator rules. App completion is not product acceptance.
...
The Hub executor also performs a bounded terminal-recovery check for requests that are already `started` or `cancel_requested`. It uses the same read-only exact-marker observer, requires the durable request's exact native thread and turn, and accepts only a recorded terminal event with nonempty final text or an authentic nonempty terminal error. The result still passes through the controller-bound queue transition and the normal Meridian result/validator path. It never derives completion from a title, report, worker marker or missing process. A running turn, identity mismatch, absent final/error text or observer ambiguity remains reserved for native-controller reconciliation.

An authentic failed turn is a non-recoverable execution terminal, not a recoverable stream transport error. Preserve `status: error` and `run_state: completed` in the Hub result and canonical history, including after restart. Status/history reads may recover a previously missed failed durable receipt only for the exact latest submitted trace and native binding, with no active owner; repeated reads must not append duplicate receipts or launch work. This reconciles terminal delivery, not product acceptance or permission.

A provider safety rejection is distinct from local filesystem permissions or missing owner consent. Keep its genuine error and partial checkout, escalate the precise restriction through the supported provider path, and do not automatically retry, rephrase, switch provider/executor, weaken safeguards, or advance dependent workers to work around it. Fixing Meridian's error propagation does not remove a provider restriction.
