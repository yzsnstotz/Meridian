...
export function buildSystemPrompt(vars: PromptVars): string {
  ...
  return [
    ...
    "Step 4. Call `run --thread-id <thread_id> --command <command_file_path> --worker <worker_id>`. The lifecycle store will update `dispatch_threads.json` and regenerate `dispatch_plan.md`; you never write plan status directly.",
