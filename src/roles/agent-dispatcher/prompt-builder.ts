...
export function buildSystemPrompt(vars: PromptVars): string {
  ...
  return [
    ...
    "",
    "5. update-status",
    `   Command: \`${TOOL_ENTRYPOINT} update-status --plan <path> --worker <id> --status <status> [--thread-id <id>]\``,
    "   Success example:",
    "   ```json",
    "   {",
    '     "ok": true,',
    ...
    "   Pass `--thread-id` when setting `in_progress` so `dispatch_threads.json` tracks the active worker thread.",
    "",
    "# Workflow Steps",
    ...
    "Step 4. Mark the worker in progress with `update-status --status in_progress --thread-id <thread_id>`, then call `run --thread-id <thread_id> --command <command_file_path> --worker <worker_id>`.",
    "Step 5. Interpret `run` results strictly by JSON shape.",
    "- `ok:true` and `data.run_state` is absent or `completed`: mark the worker `done`.",
    "- `ok:true` and `data.run_state` is `still_running` or `timeout`: leave the worker row at `🔄`, do not mark it done or failed, do not auto-kill the worker, notify a human with the worker/thread/state/summary, and pause until an explicit resume.",
    "- `ok:false`: mark the worker `failed`, record the error, and let dependency failures propagate naturally through the DAG.",
    ...
    "Step 8. Special nodes: `DELTA-CHECK` and `PR-REVIEW` run the same spawn -> update-status -> run -> kill-policy loop, then you read the report they produced. `MERGE BLOCKED` requires notify + pause for a human. `MERGE APPROVED` requires the final completion notify and stop.",
    "",
    "# Judgment Rules",
    ...
    "- `run` returning `ok:true` with `data.run_state` of `still_running` or `timeout` is a structured non-final result. Keep the worker `🔄` and do not flatten it to success text.",
